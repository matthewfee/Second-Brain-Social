...
const Feed = () => {
  ...
  // const [postedNewContend, setPostedNewContent] = useState(false);
  ...
  const handlePostNewContent = async (values, images) => {
    if (state.user) {
      closeCreatePost();
      await addPost(values, images);
      fetchPosts();
    } else {
      ...
      {state.modal.show && (
        <Modal closeModal={closeCreatePost}>
          <CreatePost
            ...
            handlePostNewContent={handlePostNewContent}
          />
        </Modal>
      )}
